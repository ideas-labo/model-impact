...
def get_elite(candidates,file,model_name,N_min,T_each = 1,T_first = 5):
    ...
    for x in scores:
        ...
        for y in scores:
            if x != y:
                anw = ttest_rel(x,y)
                if anw.pvalue <=0.05:
                    res.append(anw.statistic)
                else:
                    res.append(0)
        reses.append(res)
    for i in range(len(reses)-1,0,-1):

        if sum(j>0 for j in reses[i]) >= N_min:
            candidates.pop(i)
            scores.pop(i)  
        elif reses[i] != []:
            if min(reses[i]) > 0:
    ...
    # each_test 
    while len(candidates)>= N_min:
        # print(len(scores))
        # print(len(candidates))
        for i in range(len(candidates)):
            tmp = get_objective_score_similarly(candidates[i],file.dict_search,model_name)
            scores[i].append(tmp)
            # print(scores[i])
        reses = []
        for x in scores:
    ...
    try:
        while 1:
            ...
            for i in range(N_iter):
                ...
                # one_with_score = []
                # for j in candidates:
                #     tmp_target = get_objective_score(file.dict_search,j)
                #     one_with_score.append((j,tmp_target))
                # one_with_score = sorted(one_with_score,key= lambda x:x[1])
                # print(one_with_score)
                # # 只保留N_min个样本
                # one_with_score = one_with_score[0:N_min]
                # elite = [i[0] for i in one_with_score]
